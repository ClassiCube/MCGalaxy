...
namespace MCGalaxy.Generator.Foliage {
	...
	public abstract class ForesterTree : Tree {
		...
		public override void SetData(Random rnd, int value) { this.rnd = rnd; height = 40; }
		...
		protected void assign_value(int x, int y, int z, byte block) {
	...
	public abstract class ProceduralTree : ForesterTree {
		...
		/// </remarks>
		protected void crossection(Vec3S32 center, float radius, int diraxis, byte block) {
				...
				for (int off2 = -rad; off2 <= rad; off2++)
			{
				...
				assign_value(coord.X, coord.Y, coord.Z, block);
			}
		}
		...
		protected void foliagecluster(Vec3S32 center) {
			float[] level_radius = foliage_shape;
			foreach (float i in level_radius) {
				crossection(center, i, 1, Block.leaf);
				center.Y += 1;
			}
		}
		...
		/// </remarks>
		protected void taperedcylinder(Vec3S32 start, Vec3S32 end, float startsize, float endsize, byte block) {
			...
			for (int primoffset = 0; primoffset != endoffset; primoffset += primsign) {
				...
				crossection(coord, radius, primidx, block);
			}
		}
		...
		public override void makefoliage() {
			...
			foreach (Vec3S32 coord in foliage_coords) {
				assign_value(coord.X, coord.Y, coord.Z, Block.trunk);
			}
		}
		...
		protected void makebranches() {
			...
			foreach (Vec3S32 coord in foliage_coords) {
				...
				float endsize = 1.0f;
				taperedcylinder(startcoord, coord, (float)startsize, endsize, Block.leaf);
			}
		}
		...
		public override void maketrunk() {
			...
			if (ROOTBUTTRESSES || mangrove) {
				...
				for (int i = 0; i < num_of_buttresses; i++) {
					...
					taperedcylinder(
						new Vec3S32(thisx, starty, thisz),
						new Vec3S32(x, midy, z),
						thisbuttressradius, thisbuttressradius, Block.trunk);
				}
			} else {
			...
			taperedcylinder(
				new Vec3S32(x, starty, z),
				new Vec3S32(x, midy, z),
				startrad, midrad, Block.trunk);
			taperedcylinder(
				new Vec3S32(x, midy, z),
				new Vec3S32(x, topy, z),
				midrad, endrad, Block.trunk);
			// Make the branches
			makebranches();
		}
