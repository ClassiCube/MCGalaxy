...
namespace MCGalaxy.Network
{
    ...
    public abstract class INetSocket
    {
        ...
        INetProtocol IdentifyProtocol(byte opcode) {
            // Logger.Log(LogType.UserActivity, "dbg IP {0} opcode {1}", IP, opcode);
            if (opcode == Opcode.Handshake) {
                ...
                // Logger.Log(LogType.UserActivity, "dbg wss");
                pending.Remove(this);
        ...
        protected void HandleReceived(byte[] data, int len) {
            ...
            if (protocol == null) {
                ...
                // Logger.Log(LogType.UserActivity, "dbg proto {0}", protocol);
                if (protocol == null) return;
            }
            byte[] src;

            // Logger.Log(LogType.UserActivity, "dbg data {0}", data);
...
// This code is unfinished and experimental, and is terrible quality. I apologise in advance.
    public sealed class SecureSocket : INetSocket, INetProtocol {
        ...
        void IOThread() {
            try {
                ...
                LogSecurityLevel(ssl);
                LogSecurityServices(ssl);
                LogCertificateInformation(ssl);
                LogStreamProperties(ssl);
    ...
    static void LogSecurityLevel(SslStream stream) {
        Logger.Log(LogType.UserActivity, "Cipher: {0} strength {1}", stream.CipherAlgorithm, stream.CipherStrength);
        Logger.Log(LogType.UserActivity, "Hash: {0} strength {1}", stream.HashAlgorithm, stream.HashStrength);
        Logger.Log(LogType.UserActivity, "Key exchange: {0} strength {1}", stream.KeyExchangeAlgorithm, stream.KeyExchangeStrength);
        Logger.Log(LogType.UserActivity, "Protocol: {0}", stream.SslProtocol);
     }
     static void LogSecurityServices(SslStream stream) {
        Logger.Log(LogType.UserActivity, "Is authenticated: {0} as server? {1}", stream.IsAuthenticated, stream.IsServer);
        Logger.Log(LogType.UserActivity, "IsSigned: {0}", stream.IsSigned);
        Logger.Log(LogType.UserActivity, "Is Encrypted: {0}", stream.IsEncrypted);
     }
     static void LogStreamProperties(SslStream stream) {
        Logger.Log(LogType.UserActivity, "Can read: {0}, write {1}", stream.CanRead, stream.CanWrite);
        Logger.Log(LogType.UserActivity, "Can timeout: {0}", stream.CanTimeout);
     }
    static void LogCertificateInformation(SslStream stream) {
        Logger.Log(LogType.UserActivity, "Certificate revocation list checked: {0}", stream.CheckCertRevocationStatus);

        X509Certificate localCertificate = stream.LocalCertificate;
        if (stream.LocalCertificate != null) {
            Logger.Log(LogType.UserActivity, "Local cert was issued to {0} and is valid from {1} until {2}.",
                localCertificate.Subject,
                localCertificate.GetEffectiveDateString(),
                localCertificate.GetExpirationDateString());
         } else {
            Logger.Log(LogType.UserActivity, "Local certificate is null.");
        }
        // Display the properties of the client's certificate.
        X509Certificate remoteCertificate = stream.RemoteCertificate;
        if (stream.RemoteCertificate != null) {
        Logger.Log(LogType.UserActivity, "Remote cert was issued to {0} and is valid from {1} until {2}.",
            remoteCertificate.Subject,
            remoteCertificate.GetEffectiveDateString(),
            remoteCertificate.GetExpirationDateString());
        } else {
            Logger.Log(LogType.UserActivity, "Remote certificate is null.");
        }
    }
    }
#endif
