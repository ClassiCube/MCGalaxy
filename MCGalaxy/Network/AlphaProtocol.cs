...
{
    public class AlphaProtocol : IGameProtocol 
    {
        Player p;
        INetSocket socket;
...
#region Classic processing
        int HandleLogin(byte[] buffer, int offset, int left) {
            ...
            if (version != PROTOCOL_VERSION) {
                p.Leave("Unsupported protocol version!"); return -1;
            }
            ...
            return p.ProcessLogin(name, pass) ? size : -1;
        }
        ...
        int HandleSelfStateOnly(byte[] buffer, int offset, int left) {
            ...
            p.ProcessMovement(pos.X, pos.Y, pos.Z, rot.RotY, rot.HeadX, 0);
            return size;
        }
        ...
        int HandleSelfMove(byte[] buffer, int offset, int left) {
            ...
            Orientation rot = p.Rot;
            p.ProcessMovement((int)(x * 32), (int)(y * 32), (int)(z * 32),
        ...
        int HandleSelfLook(byte[] buffer, int offset, int left) {
            ...
            Position pos = p.Pos;
            p.ProcessMovement(pos.X, pos.Y, pos.Z,
        ...
        int HandleBlockDig(byte[] buffer, int offset, int left) {
            ...
            if (status == 3)
                p.ProcessBlockchange((ushort)x, (ushort)y, (ushort)z, 0, 0);
            return size;
        }
        ...
        int HandleBlockPlace(byte[] buffer, int offset, int left) {
            ...
            p.ProcessBlockchange((ushort)x, (ushort)y, (ushort)z, 1, block);
            return size;
        }
