...
namespace MCGalaxy.Commands {
    ...
    public sealed class CmdLogoutMessage : Command {
        ...
        public override void Use(Player p, string message) {
            ...
            CP437Writer.WriteAllText("text/logout/" + name + ".txt", args[1]);
            
            string fullName = target != null ? target.FullName : name;
            Player.SendMessage(p, "The logout message of " + fullName + " %Shas been changed to:");
            Player.SendMessage(p, args[1]);
            if (p != null)
                Server.s.Log(p.name + " changed " + name + "'s logout message to:");
            else
                Server.s.Log("(console) changed " + name + "'s logout message to:");
            Server.s.Log(args[1]);
        }
