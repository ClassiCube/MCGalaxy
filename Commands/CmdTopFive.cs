    ...
    public sealed class CmdTopFive : Command
    {
        ...
        public override void Use(Player p, string message)
        {
            ...
            if (String.Compare(message, "1", true) == 0)
            {
                ...
                Player.SendMessage(p, "TOP FIVE NUMBER OF LOGINS:");
            ...
            {
                DataTable playerDb = Database.fillData("SELECT distinct name, totaldeaths FROM Players order by totaldeaths desc limit 5");

                Player.SendMessage(p, "TOP FIVE NUMBER OF DEATHS:");
                for (int i = 0; i < playerDb.Rows.Count; i++)
                {
            ...
            {
                DataTable playerDb = Database.fillData("SELECT distinct player, money FROM Economy order by money desc limit 5");

                Player.SendMessage(p, "TOP FIVE AMOUNTS OF MONEY:");
                for (int i = 0; i < playerDb.Rows.Count; i++)
                {
            ...
            {
                DataTable playerDb = Database.fillData("SELECT distinct name, firstlogin FROM Players order by firstlogin asc limit 5");
            ...
            {
                DataTable playerDb = Database.fillData("SELECT distinct name, lastlogin  FROM Players order by lastlogin desc limit 5");
            ...
            {
                DataTable playerDb = Database.fillData("SELECT distinct name, totalblocks FROM Players order by totalblocks desc limit 5");

                Player.SendMessage(p, "TOP FIVE NUMBER OF BLOCKS MODIFIED:");
                for (int i = 0; i < playerDb.Rows.Count; i++)
                {
            ...
            {
                DataTable playerDb = Database.fillData("SELECT distinct name, totalkicked FROM Players order by totalkicked desc limit 5");

                Player.SendMessage(p, "TOP FIVE NUMBER OF KICKS:");
                for (int i = 0; i < playerDb.Rows.Count; i++)
                {
